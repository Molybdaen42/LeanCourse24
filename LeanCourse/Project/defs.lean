...
noncomputable def dist (z : ℂ) (l : line) : ℝ := 
...
noncomputable def O6 (z₁ z₂ : ℂ) (h : z₁ ≠ z₂) (l₁ l₂ : line) : Set line := 
  {t : line | 
  t.vec ≠ l₁.vec ∧ t.vec ≠ -l₁.vec ∧ -- t.vec is not orthogonal to l₁.vec.
  t.vec ≠ l₂.vec ∧ t.vec ≠ -l₂.vec ∧ -- t.vec is not orthogonal to l₂.vec.
  l₁.z₁ ∈ t.points ∧ Complex.abs (t.z₁-z₁) = dist t.z₁ l₁ ∧ -- t is intersecting the parabola of z₁ and l₁ ...
  (∀ z ∈ t.points, Complex.abs (z-z₁) = dist z l₁ → z = t.z₁) ∧ -- ... in exactly one point.
  l₁.z₁ ∈ t.points ∧ Complex.abs (t.z₂-z₂) = dist t.z₂ l₂ ∧ -- t is intersecting the parabola of z₂ and l₂ ...
  (∀ z ∈ t.points, Complex.abs (z-z₂) = dist z l₂ → z = t.z₂) -- ... in exactly one point.
  }
