...
lemma i_in_𝕆 : Complex.I ∈ 𝕆 := by
  ...
  have h1 : reAxis ∈ 𝕆_lines := by
    simp [𝕆_lines]
    -- it's in the lines of 𝕆₁
    use 1
    -- Want to use O1(0, 1)
    -- simp does everything
    simp [generate_lines]
  have h3 : imAxis ∈ 𝕆_lines := by
    simp [𝕆_lines]
    -- imAxis is generated by elements of 𝕆₁, so it lies in 𝕆₂
    use 2
    -- Want to use O4(0, reAxis)
